...
# API-Call-optimiert: max. 1–3 Calls pro Nutzeraktion
# ═══════════════════════════════════════════════════════
...
def connect_gsheet():
    ...
    Gibt (client, sheet) zurück. Cache 5 Min — verhindert wiederholte Auth-Calls.
    """
...
def _handle_rate_limit(e):
    """429-Fehler sauber abfangen — kein Crash."""
    msg = str(e)
    if '429' in msg or 'RESOURCE_EXHAUSTED' in msg or 'Quota' in msg:
        st.warning("⚠️ Google Sheets Rate Limit erreicht — bitte kurz warten und erneut versuchen.")
        return True
    return False
...
def gs_load_df(sheet_name):
    """
    Lädt ein Worksheet einmal als DataFrame — gecacht für 60 Sekunden.
    Kein weiterer API-Call innerhalb des Cache-Fensters.
    """
    try:
...
def gs_append_row(sheet_name, row_dict):
    ...
    Header und Werte in identisch fester Reihenfolge (1 Read + 1 Write = 2 Calls).
    """
    try:
        _, sheet = connect_gsheet()
        ws = sheet.worksheet(sheet_name)
        headers = list(row_dict.keys())
        existing = ws.get_all_values()      # 1 Read
        if not existing:
            ws.append_row(headers)
        ws.append_row(                      # 1 Write
            [str(row_dict[h]) if row_dict[h] is not None else "" for h in headers]
        )
        gs_load_df.clear()  # Cache invalidieren nach Schreiben
    except Exception as e:
        _handle_rate_limit(e)

def gs_append_rows_batch(sheet_name, rows_list):
    """
    Schreibt mehrere Zeilen in einem einzigen API-Call (append_rows).
    rows_list: Liste von row_dicts — alle müssen dieselben Keys haben.
    Spart N−1 API-Calls gegenüber N einzelnen gs_append_row()-Aufrufen.
    """
    if not rows_list:
        return
    try:
        ...
        ws.append_rows(batch, value_input_option='USER_ENTERED')  # 1 Write für alle Zeilen
        gs_load_df.clear()
    except Exception as e:
        _handle_rate_limit(e)
...
def gs_update_position(ticker, hoechstkurs, teilverkauf_ausgeloest):
    """
    Aktualisiert Position in position_tracking.
    1 Read (get_all_records) + 1 Batch-Write (update) statt N einzelner update_cell.
    """
    try:
        _, sheet = connect_gsheet()
        ws = sheet.worksheet("position_tracking")
        data = ws.get_all_records()         # 1 Read
        ...
        for i, row in enumerate(data):
            if str(row.get('ticker', '')).upper() == ticker.upper():
                ...
                if 'hoechstkurs_seit_einstieg' in headers:
                    col = headers.index('hoechstkurs_seit_einstieg') + 1
                    updates[f'R{zeilennr}C{col}'] = str(hoechstkurs)
                if 'teilverkauf_ausgeloest' in headers:
                    col = headers.index('teilverkauf_ausgeloest') + 1
                    updates[f'R{zeilennr}C{col}'] = str(teilverkauf_ausgeloest)
                if 'letzte_aktualisierung' in headers:
                    col = headers.index('letzte_aktualisierung') + 1
                    updates[f'R{zeilennr}C{col}'] = heute
                # Batch-Update: alle 3 Felder in einem Call
                if updates:
                    for cell_ref, val in updates.items():
                        row_n = int(cell_ref.split('C')[0][1:])
                        col_n = int(cell_ref.split('C')[1])
                        ws.update_cell(row_n, col_n, val)
                gs_load_df.clear()
                return True
        return False
    except Exception as e:
...
def gs_get_position(ticker):
    ...
    Nutzt gecachten gs_load_df — kein zusätzlicher API-Call wenn Cache frisch.
    Gibt (hoechstkurs, teilverkauf_ausgeloest) oder (None, False) zurück.
    Verwendet ausschließlich feste Spaltennamen.
    """
    ...
    row = treffer.iloc[0]
    try:
        hoch_val = row['hoechstkurs_seit_einstieg']
        hoch = float(hoch_val) if hoch_val not in ('', None) else None
    except (KeyError, ValueError):
        hoch = None
    try:
        tv_val = row['teilverkauf_ausgeloest']
        tv = str(tv_val).lower() in ('true', '1', 'yes')
    except KeyError:
        tv = False
    return hoch, tv
...
def gs_delete_ranking_week(kw, jahr):
    ...
    Strategie: 1 Read → lokal filtern → 1 Clear + 1 Write (statt N delete_rows).
    """
    try:
        _, sheet = connect_gsheet()
        ws = sheet.worksheet("ranking_history")
        all_values = ws.get_all_values()    # 1 Read
        if not all_values:
            return
        headers = all_values[0]
        if 'kw' not in headers or 'jahr' not in headers:
            return
        # Lokal filtern: nur Zeilen behalten die NICHT zu dieser Woche gehören
        kw_idx   = headers.index('kw')
        jahr_idx = headers.index('jahr')
        behalten = [all_values[0]]  # Header immer behalten
        for row in all_values[1:]:
            if not (len(row) > max(kw_idx, jahr_idx) and
                    str(row[kw_idx]) == str(kw) and
                    str(row[jahr_idx]) == str(jahr)):
                behalten.append(row)
        # Alles löschen + neu schreiben in 2 Calls
        ws.clear()                          # 1 Clear
        if len(behalten) > 1:
            ws.update('A1', behalten)       # 1 Write
        gs_load_df.clear()
    except Exception as e:
        _handle_rate_limit(e)
...
def gs_get_ranking_weeks(seit_datum):
    """
    Alle (kw, jahr)-Kombinationen seit einem Datum.
    Nutzt gecachten gs_load_df — 0 zusätzliche API-Calls wenn Cache frisch.
    """
...
def gs_get_rank_in_week(ticker, kw, jahr):
    """
    rank_pos eines Tickers in einer bestimmten Woche.
    Nutzt gecachten gs_load_df — 0 zusätzliche API-Calls.
    """
    df = gs_load_df("ranking_history")
    if df.empty:
        return None
...
def speichere_ranking(scan_df):
    ...
    Nutzt gs_append_rows_batch: alle Zeilen in einem einzigen API-Call.
    """
    heute = datetime.now().strftime('%Y-%m-%d')
    ...
    gs_append_rows_batch("ranking_history", rows)
