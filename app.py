"""
HF System — Privates Hedgefonds Dashboard v2.1
Korrekturen:
- Gewinnkonstanz: 7140 Szenarien (alle Kauf/Verkauf-Kombinationen)
- Sidebar für Satelliten-Filter
- 50 Satellitenwerte anzeigen
- VIX automatisch beim Laden
"""
...
def get_vix():
    """VIX automatisch beim App-Start laden — stündlich gecacht."""
    try:
...
# ═══════════════════════════════════════════════════════
# CHAMPIONS SCORE — POTENZFORMEL (KORRIGIERT)
# ═══════════════════════════════════════════════════════

def calc_geopak(close):
    """Geometrische Per-Annum-Kursentwicklung — Anfangskurs = Ø erste 12 Monate."""
    if len(close) < 24:
        return None
...
def calc_konstanz(close):
    """
    KORREKTE Formel: 7140 Ein-und-Ausstiegs-Szenarien.
    7140 = 120 * 119 / 2 = alle möglichen Kauf/Verkauf-Kombinationen aus 120 Monaten.
    Zählt wie viele dieser Kombinationen einen positiven Kursgewinn ergeben hätten.
    """
    if len(close) < 24:
        return None
...
def calc_verlust(close):
    """
    Zeitgewichtete Verlust-Ratio.
    Jüngere Monate erhalten höheres Gewicht (Monatsfaktor 1 bis n).
    """
    if len(close) < 24:
        return None
...
def calc_score(g, k, v):
    """Potenzformel: GeoPAK10^0.95 × (1/max(VR,0.5))^1.2 × (GK/100)^1.5"""
...
def ampel(vix, osc):
    ...
    p = max(0, vp + adj)
    if p >= 4: return p, '#16a34a', '#f0fdf4', 'GRÜNES LICHT — Vollgas'
    if p == 3: return p, '#ca8a04', '#fefce8', 'GELBES LICHT — Vorsicht'
    if p == 1: return p, '#ea580c', '#fff7ed', 'ORANGE — Defensiv'
    return p, '#dc2626', '#fef2f2', 'ROTES LICHT — Cash'
...
@st.cache_data(ttl=86400)
def scan(ticker, min_vol, max_dd):
    try:
        spy = get_daily("SPY", 2)
        ...
        # GD200 Pflichtfilter
        gd200 = h['Close'].tail(200).mean() if len(h) >= 200 else h['Close'].mean()
        if k <= gd200:
            return None

        # GD50 berechnen
        gd50 = h['Close'].tail(50).mean() if len(h) >= 50 else h['Close'].mean()
        gd50_signal = "✅ Über GD50" if k > gd50 else "❌ Unter GD50"
        ...
        # MOM260 + MOM130 (AQR Lag: letzter Monat ausgeschlossen)
        lag = 22
        ref = h['Close'].iloc[-lag] if len(h) > lag else h['Close'].iloc[-1]
        ...
        # RS z-Score
        ret_l = [(h['Close'].iloc[-i]/h['Close'].iloc[-(i+130)]-1)*100
                 for i in range(130, min(len(h), 260))]
        ...
        return {
            ...
            'Kurs': round(k, 2),
            'GD200': round(gd200, 2),
            'GD200 Abstand %': round(((k-gd200)/gd200)*100, 1),
...
vix = get_vix()  # Automatisch beim App-Start
...
st.markdown(
    ...
    '<span style="font-size:0.72rem;color:#718096;margin-left:12px">Privates Hedgefonds Dashboard v2.1</span>'
    '</div>'
...
with tab1:
    ...
    if tickers_list:
        with st.spinner("Lade Champions-Oszillator..."):
    ...
    pos, farbe, bg_hell, text = ampel(vix, osc_wert)
    ...
    st.markdown(
        ...
        f'VIX: <b>{vt}</b> · Oszillator: <b>{osc_txt}</b></div>'
        f'</div></div></div>',
        unsafe_allow_html=True
    )

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("VIX", vt, "Automatisch geladen")
    m2.metric("Oszillator", osc_txt, f"{osc_u}/{osc_g} über GD200")
    m3.metric("Satelliten max.", str(pos), "VIX + Oszillator")
    ...
    with col_links:
        ...
        for label, p, f, aktiv in [
            ...
            st.markdown(
                ...
                f'<span style="font-family:DM Mono,monospace;font-size:0.82rem;color:{f};width:90px">{label}</span>'
                f'<span style="font-size:1.1rem;font-weight:700;color:{f};width:25px">{p}</span>'
                f'<span style="font-size:0.78rem;color:#718096">Positionen{tag}</span></div>',
                unsafe_allow_html=True
            )
...
with tab2:
    ...
    st.markdown(
        ...
        'eine Zeile pro Champion.</span></div>',
        unsafe_allow_html=True
    )
    ...
    if champions_df.empty:
        ...
        st.markdown("---")
        st.markdown("### Potenzformel — Korrekte Berechnung")
        st.markdown(
            ...
            '</div>'
            '<div style="background:#f0fdf4;border:1px solid #86efac;border-radius:6px;padding:10px 14px;font-size:0.78rem;color:#15803d;margin-bottom:16px">'
            '✅ <b>Gewinnkonstanz-Formel korrigiert:</b> 7140 Szenarien (alle möglichen Kauf/Verkauf-Kombinationen aus 120 Monaten = 120×119÷2). '
            'Zählt wie viele dieser Kombinationen positiv waren — ergibt realistische Werte von 80–98%.'
            '</div>',
            unsafe_allow_html=True
        )
        ...
        with col_e:
            ...
            if st.button("Score berechnen"):
                if ticker_sel:
                    with st.spinner(f"Berechne {ticker_sel} (7140 Szenarien)..."):
        ...
        with col_a:
            ...
            if st.button("🚀 Alle berechnen", key="btn_all"):
                ...
                for i, row in champions_df.iterrows():
                    ...
                    stat.caption(f"Berechne {name} ({ticker}) — 7140 Szenarien...")
                ...
                if results:
                    ...
                    st.success(f"✅ {len(res_df)} Champions — Konstanz jetzt korrekt 80–98%")
                    st.dataframe(res_df, use_container_width=True, height=500, hide_index=True)

# ═══════════════════════════════════════════════════════
# TAB 3 — SATELLITEN (mit Sidebar-Filter)
# ═══════════════════════════════════════════════════════
...
with tab3:
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("**Volumen & Risiko**")
        min_vol = st.number_input("Min. Ø Volumen (60T)", value=4_000_000, step=500_000, format="%d")
        max_dd = st.slider("Max. Drawdown 52W (%)", min_value=-80, max_value=-10, value=-50)

        st.markdown("---")
        st.markdown("**Momentum-Filter**")
        only_above_spy = st.checkbox("Nur Aktien > SPY (130T)", value=True)
        only_above_gd200 = st.checkbox("Nur Aktien > GD200", value=True)

        st.markdown("---")
        st.markdown("**Anzeige**")
        top_n = st.slider("Top N anzeigen", min_value=10, max_value=50, value=50)
        ...
        st.markdown(
            ...
            '<b>Aktueller VIX:</b> ' + vt + '<br>'
            '<b>Erlaubte Positionen:</b> ' + str(vix_pos(vix)) + '<br>'
            '<b>Einstieg je Position:</b> € 2.500'
            '</div>',
            unsafe_allow_html=True
        )

    # HAUPTBEREICH SATELLITEN
    ...
    sa2.metric("Max. Positionen", str(vix_pos(vix)), "VIX-gesteuert")
    sa3.metric("Einstieg", "€ 2.500", "Fix je Position")
    sa4.metric("Top N", str(top_n), "Werden angezeigt")
    ...
    if 'universe_df' not in st.session_state:
        st.markdown(
            ...
            '<span style="font-size:0.78rem;color:#166534">CSV mit Spalten <code>Ticker</code> und optional <code>Name</code> — '
            'oder nutzen Sie den Filter in der ◀ Sidebar links</span></div>',
            unsafe_allow_html=True
        )
        uni_upload = st.file_uploader("Universe CSV (Hauptbereich)", type=['csv'], key="uni_main", label_visibility="collapsed")
        if uni_upload:
        ...
        if 'Ticker' in uni_df.columns:
            ...
            name_map = dict(zip(uni_df['Ticker'], uni_df.get('Name', uni_df['Ticker']))) if 'Name' in uni_df.columns else {}

            col_scan, col_clear = st.columns([1, 4])
            with col_scan:
                scan_btn = st.button("🔍 Scanner starten", key="btn_scan")
            with col_clear:
                if st.button("🗑️ Universum zurücksetzen", key="btn_clear"):
            ...
            if scan_btn:
                ...
                for i, t in enumerate(uni_tickers):
                    ...
                    if i % 50 == 0:
                        status2.caption(f"Scanne... {i+1}/{limit} | Kandidaten bisher: {len(results)}")
                    r = scan(t, min_vol, max_dd)
                    if r:
                ...
                if results:
                    ...
                    cols_order = ['#', 'Name', 'Ticker', 'Kurs', 'GD200', 'GD200 Abstand %',
                                  'GD50 Signal', 'MOM260 %', 'MOM130 %', 'RS Score',
                                  'RS vs SPY', 'RS z-Score', 'Drawdown %', 'Score ⭐']
                    scan_df = scan_df[[c for c in cols_order if c in scan_df.columns]]

                    st.success(f"✅ {len(results)} Kandidaten gefunden — Top {min(top_n, len(results))} angezeigt")

                    # Branchen-Warnung
                    st.info(
                        f"⚠️ **Klumpen-Check:** Max. 2 Positionen pro Branche. "
                        f"VIX {vt} → max. {vix_pos(vix)} Positionen aktiv. "
                        f"Einstieg je Position: **€ 2.500 fix**."
                    )
                    ...
                    # Ausstiegs-Erinnerung
                    with st.expander("📋 Ausstiegs-Trigger (2 von 3 = Verkaufen)"):
                        st.markdown(
                            "🔴 **Top-15-Exit:** Position fällt aus Top 15 → sofortiger Verkauf\n\n"
                            "🔴 **Strukturbruch:** Signal 1 (Chart) + Signal 2 (GD50) + Signal 3 (RS) — 2 von 3 aktiv = Verkauf\n\n"
                            "🔴 **Notfall -15%:** Intraweek Verlust > -15% vom Einstieg → Sofortverkauf\n\n"
                            "ℹ️ **Hinweis:** Lumentum (LITE), Western Digital (WDC) und ähnliche Werte "
                            "erscheinen nicht, wenn sie aktuell unter dem GD200 notieren — das ist korrekt "
                            "nach Ihrer Strategie. Der GD200-Pflichtfilter schützt vor strukturellen Abwärtstrends."
                        )
                else:
...
with tab5:
    ...
    if champions_df2.empty:
        ...
        o1.metric("Oszillator", osc_txt2, "Stark" if ow and ow>=80 else "Positiv" if ow and ow>=60 else "Gemischt" if ow and ow>=40 else "Schwach")
        o2.metric("Über GD200", f"{ou}", f"von {og} Champions")
        o3.metric("Unter GD200", f"{og-ou}", f"von {og} Champions")
        o4.metric("VIX", vt, "Automatisch geladen")
        ...
        # Aktienbrief-Wert
        st.info("📋 **Börsenverlag Aktienbrief Nr. 627 (16.03.2026):** Champions-Oszillator = **37%** → Empfehlung: ca. 1/3 des Kapitals in Champions investiert sein.")
        ...
        for stand, markt, emp, f in [
            ...
            aktiv = ow and (
                (stand=="> 80%" and ow>=80) or (stand=="60–80%" and 60<=ow<80) or
                (stand=="40–60%" and 40<=ow<60) or (stand=="20–40%" and 20<=ow<40) or (stand=="< 20%" and ow<20)
            )
            bd = f if aktiv else "#e2e8f0"
            bg = f"{f}12" if aktiv else "#ffffff"
            st.markdown(
                ...
                f'<span style="color:#2d3748;width:150px;font-size:0.82rem">{markt}</span>'
        ...
        if not od.empty:
            with st.expander("📋 Alle Champions vs. GD200"):
...
with tab6:
    ...
    st.markdown(
        ...
        f'VIX: <b>{vt}</b> · Erlaubte Positionen: <b>{vix_pos(vix)}</b>'
        f'</div>',
        unsafe_allow_html=True
    )
    ...
    with cl1:
        ...
        c2 = st.checkbox("Signal 1 — Chart-Struktur intakt?")
        c3 = st.checkbox("Signal 2 — Kurs über GD50?")
        c4 = st.checkbox("Signal 3 — RS nicht >5 Plätze gefallen?")
    ...
    with cl2:
        ...
        c11 = st.checkbox("Champions-Oszillator abgelesen?")
        c12 = st.checkbox("Ampelsignal notiert?")
        st.markdown("**🔍 Neue Kandidaten**")
        c13 = st.checkbox("Top 50 Screening geprüft?")
    ...
    if st.button("✅ Checkliste speichern"):
        checklist_save(vix, osc_wert if 'osc_wert' in dir() else None, vix_pos(vix), notizen_cl)
...
with tab7:
    ...
    with jc1:
        ...
        j_grund = st.text_area("Einstiegsbegründung", placeholder="Ranking, Chart, Score...", height=80)
        j_trigger = st.text_input("Ausstiegs-Trigger", placeholder="z.B. Top-15-Exit")
...
st.markdown(
    ...
    'HF · System v2.1 — Privates Hedgefonds Dashboard — Streng vertraulich'
    '</p></div>',
    unsafe_allow_html=True
)
