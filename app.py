...
with c1:
    st.markdown(
        ...
        '<p style="font-family:DM Mono,monospace;font-size:0.6rem;color:#5a6380;margin:0;letter-spacing:0.12em">PRIVATES HEDGEFONDS DASHBOARD</p>',
        unsafe_allow_html=True
    )
with c2:
    st.markdown(
        f'<p style="font-family:DM Mono,monospace;font-size:0.72rem;color:#5a6380;margin-top:14px">'
...
with c3:
    ...
    st.markdown(
        ...
        f'<span style="font-family:DM Mono,monospace;font-size:0.58rem;color:#5a6380">VIX</span><br>'
        f'<span style="font-family:Syne,sans-serif;font-size:1.7rem;font-weight:800;color:{vc}">{vt}</span>'
        f'</div>',
        unsafe_allow_html=True
    )

st.markdown("<hr style='border-color:#1e2535;margin:8px 0 16px 0'>", unsafe_allow_html=True)
...
with tab1:
    ...
    st.markdown(
        ...
        f'<div style="font-family:DM Mono,monospace;font-size:0.68rem;color:#5a6380;margin-top:8px">'
    ...
    with col_vix:
        ...
        for label, p, f, aktiv in [
            ...
            bg = f"{f}15" if aktiv else "#12161f"
            bd = f if aktiv else "#1e2535"
            ...
            st.markdown(
                ...
                f'<span style="font-size:0.73rem;color:#5a6380">Positionen{tag}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    with col_alloc:
        ...
        for name, pct, f in [
            ...
            st.markdown(
                ...
                f'<span style="font-family:DM Mono,monospace;font-size:0.68rem;color:#5a6380;width:115px">{name}</span>'
                f'<div style="flex:1;height:7px;background:#1e2535;border-radius:4px;overflow:hidden">'
...
with tab3:
    ...
    st.markdown(
        '<div style="background:#12161f;border:1px solid #1e2535;border-radius:8px;padding:16px">'
...
with tab4:
    ...
    if st.button("Kapitalfluss berechnen", key="btn_kf"):
        ...
        st.markdown(
            f'<div style="background:#12161f;border:1px solid #1e2535;border-radius:8px;padding:20px;margin-top:12px">'
            f'<p style="font-family:DM Mono,monospace;font-size:0.62rem;color:#5a6380;margin-bottom:12px">KAPITALFLUSS-HIERARCHIE</p>'
            ...
            f'<p style="margin:8px 0"><span style="color:#5a6380;font-weight:700">Cash</span> — Parken: <b>€ {max(0, rest - ziel*0.22):,.0f}</b></p>'
            f'</div>',
            unsafe_allow_html=True
        )
    ...
    for i, (name, pct, f) in enumerate([
        ...
        with ak[i]:
            st.markdown(
                f'<div style="background:#12161f;border:1px solid #1e2535;border-radius:10px;padding:14px;text-align:center">'
                f'<p style="font-family:DM Mono,monospace;font-size:0.58rem;color:#5a6380;margin:0 0 4px">{name.upper()}</p>'
                f'<p style="font-family:Syne,sans-serif;font-size:1.5rem;font-weight:800;color:{f};margin:0">{pct}%</p>'
                f'<p style="font-family:DM Mono,monospace;font-size:0.75rem;color:#e8eaf0;margin:4px 0 1px">€ {depot*pct/100:,.0f}</p>'
                f'<p style="font-size:0.65rem;color:#5a6380;margin:0">+ € {spar*pct/100:,.0f}/Mo.</p>'
                f'</div>',
                unsafe_allow_html=True
            )
...
with tab5:
    ...
    if osc_wert:
        fig = go.Figure(go.Indicator(
            mode="gauge+number",
            value=osc_wert,
            title={'text': "Champions-Oszillator", 'font': {'color': '#e8eaf0', 'size': 15}},
            number={'suffix': '%', 'font': {'color': '#00d4aa', 'size': 38}},
            gauge={
                'axis': {'range': [0, 100], 'tickcolor': '#5a6380'},
                ...
                'steps': [
                    ...
                    {'range': [40, 60],  'color': '#1e2535'},
                    {'range': [60, 80],  'color': '#0d2b1e'},
                    {'range': [80, 100], 'color': '#0a2318'},
                ],
            }
        ))
        fig.update_layout(paper_bgcolor='#0a0c10', font={'color': '#e8eaf0'}, height=280, margin=dict(t=40,b=0))
    ...
    for stand, markt, empfehlung, f in [
        ...
        bd = f if aktiv else "#1e2535"
        bg = f"{f}12" if aktiv else "#12161f"
        st.markdown(
            ...
            f'<span style="color:#e8eaf0;width:160px;font-size:0.82rem">{markt}</span>'
            f'<span style="color:#5a6380;font-size:0.78rem">{empfehlung}</span>'
...
with tab6:
    ...
    st.markdown(
        f'<p style="font-family:DM Mono,monospace;font-size:0.72rem;color:#5a6380">'
    ...
    st.markdown(
        f'<div style="background:#12161f;border:1px solid #1e2535;border-radius:8px;padding:14px;margin:12px 0">'
        f'<div style="display:flex;justify-content:space-between;margin-bottom:7px">'
        f'<span style="font-family:DM Mono,monospace;font-size:0.72rem;color:#5a6380">FORTSCHRITT</span>'
        f'<span style="font-family:Syne,sans-serif;font-weight:700;color:{fc}">{done}/{len(checks)}</span>'
        f'</div>'
        f'<div style="height:7px;background:#1e2535;border-radius:4px;overflow:hidden">'
    ...
    with jc2:
        ...
        if trades.empty:
            ...
            for _, row in trades.head(10).iterrows():
                ...
                status_c = "#00d4aa" if pd.isna(row.get('ausstieg_datum')) else "#5a6380"
                st.markdown(
                    f'<div style="background:#12161f;border:1px solid #1e2535;border-radius:8px;'
                    ...
                    f'<span style="font-family:DM Mono,monospace;font-size:0.62rem;color:#5a6380">{row["datum"]} · {row["typ"]}</span>'
                    f'</div>'
                    f'<p style="font-size:0.78rem;color:#5a6380;margin:2px 0">{str(row.get("begruendung",""))[:90]}</p>'
                    f'<p style="font-size:0.72rem;color:#5a6380;margin:2px 0">Exit: {row.get("ausstieg_trigger","—")}{pnl}</p>'
                    f'</div>',
                    unsafe_allow_html=True
                )
...
# Footer
st.markdown(
    '<div style="text-align:center;padding:24px 0 8px;border-top:1px solid #1e2535;margin-top:24px">'
    '<p style="font-family:DM Mono,monospace;font-size:0.6rem;color:#5a6380">'
