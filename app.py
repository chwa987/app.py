...
def _get_hoechstkurs(ticker):
    """
    Holt den gespeicherten Höchstkurs seit Einstieg und Teilverkauf-Status.
...
def _update_hoechstkurs(ticker, kurs_aktuell):
    ...
    teilverkauf_ausgeloest bleibt unverändert (wird separat gesetzt).
    """
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
...
def setze_einstieg(ticker, einstieg_kurs):
    ...
    und setzt teilverkauf_ausgeloest auf 0 (neu gestartet).
    """
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
...
def trade_add(ticker, name, typ, kurs, betrag, grund, trigger):
    """Trade in SQLite speichern + CSV-Backup anhängen."""
    init_db()
    # SQLite
    conn = sqlite3.connect(DB)
    cur = conn.cursor()
    ...
    conn.commit()
    # CSV-Backup: neuen Eintrag anhängen
    try:
        ...
        with open(TRADES_CSV, 'a', newline='', encoding='utf-8') as f:
            ...
            w.writerow([neue_id, datetime.now().strftime('%Y-%m-%d'),
                        ticker.upper(), name, typ, kurs, betrag, grund, trigger,
                        None, None, None, None])
    except Exception:
        pass
    conn.close()
...
def trades_load():
    """Trades laden — SQLite primär, CSV als Fallback."""
    init_db()
    try:
        conn = sqlite3.connect(DB)
        cur = conn.cursor()
