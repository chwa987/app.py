...
def gs_append_row(sheet_name, row_dict):
    """Hängt eine Zeile an ein Worksheet an. Header und Werte in identisch fester Reihenfolge."""
    _, sheet = connect_gsheet()
    ws = sheet.worksheet(sheet_name)
    ...
    ws.append_row([str(row_dict[h]) if row_dict[h] is not None else "" for h in headers])
