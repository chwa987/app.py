...
def connect_gsheet():
    ...
    Gibt (client, sheet) zurück oder (None, None) bei Fehler.
    Cached für 5 Minuten — verhindert zu viele Auth-Aufrufe.
    """
...
def _gsheets_ok():
    """Prüft ob Google Sheets verfügbar und konfiguriert ist."""
    if not GSHEETS_AVAILABLE:
        return False
    try:
...
def gs_load_df(sheet_name):
    """
    Lädt ein Worksheet als DataFrame.
    Gibt leeren DataFrame zurück bei Fehler.
    """
    try:
...
def gs_append_row(sheet_name, row_dict):
    """
    Hängt eine Zeile an ein Worksheet an.
    Legt Header an falls Worksheet leer.
...
def gs_update_position(ticker, hoechstkurs, teilverkauf_ausgeloest):
    """
    Aktualisiert oder legt position_tracking in Google Sheets an.
    Sucht nach Ticker-Zeile und überschreibt sie.
    """
    try:
        _, sheet = connect_gsheet()
        if sheet is None:
        ...
        for i, row in enumerate(data):
            if str(row.get('ticker', '')).upper() == ticker.upper():
                # Zeile gefunden — aktualisieren (i+2: 1 für Header, 1 für 0-Index)
                zeilennr = i + 2
                # Spalten: ticker, einstieg_datum, einstieg_kurs, hoechstkurs, teilverkauf_ausgeloest, letzte_aktualisierung
                headers = ws.row_values(1)
                if 'hoechstkurs_seit_einstieg' in headers:
                    col_hoch = headers.index('hoechstkurs_seit_einstieg') + 1
                    ws.update_cell(zeilennr, col_hoch, str(hoechstkurs))
                if 'teilverkauf_ausgeloest' in headers:
                    col_tv = headers.index('teilverkauf_ausgeloest') + 1
                    ws.update_cell(zeilennr, col_tv, str(teilverkauf_ausgeloest))
                if 'letzte_aktualisierung' in headers:
                    col_lakt = headers.index('letzte_aktualisierung') + 1
                    ws.update_cell(zeilennr, col_lakt, heute)
                return True
...
def gs_get_position(ticker):
    """
    Holt position_tracking-Eintrag für einen Ticker.
    ...
    try:
        ...
        if treffer.empty:
            return None, False
        row = treffer.iloc[0]
        # Hoch-Spalte (kann unterschiedlich heißen)
...
def gs_delete_ranking_week(kw, jahr):
    """Löscht alle Ranking-Einträge einer bestimmten ISO-Woche aus Google Sheets."""
    try:
        _, sheet = connect_gsheet()
        if sheet is None:
            return
        ...
        data = ws.get_all_records()
        headers = ws.row_values(1)
        kw_col   = headers.index('kw') + 1   if 'kw'   in headers else None
        jahr_col = headers.index('jahr') + 1 if 'jahr' in headers else None
        if kw_col is None or jahr_col is None:
            return
        # Rückwärts löschen um Index-Verschiebung zu vermeiden
        zeilen_loeschen = []
        for i, row in enumerate(data):
            if (str(row.get('kw', '')) == str(kw) and
                    str(row.get('jahr', '')) == str(jahr)):
                zeilen_loeschen.append(i + 2)  # +2: Header + 0-Index
        for z in sorted(zeilen_loeschen, reverse=True):
            ws.delete_rows(z)
    except Exception:
        pass
...
def gs_get_ranking_weeks(seit_datum):
    """
    Lädt alle gespeicherten (kw, jahr)-Kombinationen seit einem Datum.
    Gibt Liste von Tuples zurück.
    """
    try:
        ...
        if df.empty:
            return []
        wochen = df[['kw', 'jahr']].drop_duplicates()
        wochen = wochen.sort_values(['jahr', 'kw'])
        return [(int(r['kw']), int(r['jahr'])) for _, r in wochen.iterrows()]
    except Exception:
        return []

def gs_get_rank_in_week(ticker, kw, jahr):
    """Gibt rank_pos eines Tickers in einer bestimmten Woche zurück oder None."""
    try:
        df = gs_load_df("ranking_history")
        if df.empty:
            return None
...
def _get_hoechstkurs(ticker):
    """
    Holt den gespeicherten Höchstkurs und Teilverkauf-Status.
    GSheets primär → SQLite als Fallback.
    Gibt zurück: (hoechstkurs, teilverkauf_ausgeloest) oder (None, False).
    """
    # GSheets
    if _gsheets_ok():
        hoch, tv = gs_get_position(ticker)
        if hoch is not None:
            return hoch, tv
    # SQLite Fallback
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT hoechstkurs, teilverkauf_ausgeloest FROM position_tracking WHERE ticker = ? LIMIT 1",
            (ticker,)
        )
        row = cur.fetchone()
        conn.close()
        if row:
            return (float(row[0]) if row[0] else None, bool(row[1]))
    except Exception:
        pass
    return None, False
...
def _update_hoechstkurs(ticker, kurs_aktuell):
    """
    Aktualisiert den Höchstkurs.
    GSheets primär → SQLite als Fallback (immer beide versuchen für Konsistenz).
    """
    heute = datetime.now().strftime('%Y-%m-%d')
    # GSheets
    if _gsheets_ok():
    ...
    # SQLite Fallback (immer)
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        cur = conn.cursor()
        cur.execute("SELECT hoechstkurs FROM position_tracking WHERE ticker = ? LIMIT 1", (ticker,))
        row = cur.fetchone()
        if row is None:
            conn.execute(
                "INSERT INTO position_tracking "
                "(ticker, einstieg_datum, einstieg_kurs, hoechstkurs, teilverkauf_ausgeloest, letzte_aktualisierung) "
                "VALUES (?, ?, ?, ?, 0, ?)",
                (ticker, heute, kurs_aktuell, kurs_aktuell, heute)
            )
        else:
            bisheriges_hoch = float(row[0]) if row[0] else 0.0
            if kurs_aktuell > bisheriges_hoch:
                conn.execute(
                    "UPDATE position_tracking SET hoechstkurs = ?, letzte_aktualisierung = ? WHERE ticker = ?",
                    (kurs_aktuell, heute, ticker)
                )
        conn.commit()
        conn.close()
    except Exception:
...
def setze_einstieg(ticker, einstieg_kurs):
    """
    Einstieg manuell registrieren — setzt Hoch seit Einstieg auf Einstiegskurs.
    GSheets primär → SQLite als Fallback.
    """
    heute = datetime.now().strftime('%Y-%m-%d')
    # GSheets
    if _gsheets_ok():
        gs_append_row("position_tracking", {
            "ticker": ticker.upper(),
            "einstieg_datum": heute,
            "einstieg_kurs": einstieg_kurs,
            "hoechstkurs_seit_einstieg": einstieg_kurs,
            "teilverkauf_ausgeloest": False,
            "letzte_aktualisierung": heute
        })
    # SQLite Fallback
    try:
        ...
        conn.execute(
            ...
            "(ticker, einstieg_datum, einstieg_kurs, hoechstkurs, teilverkauf_ausgeloest, letzte_aktualisierung) "
            "VALUES (?, ?, ?, ?, 0, ?)",
            (ticker, heute, einstieg_kurs, einstieg_kurs, heute)
        )
        conn.commit()
        conn.close()
    except Exception:
...
def scan(ticker, min_vol=5_000_000, min_kurs=10.0):
    ...
    try:
        ...
        # Kein Fallback auf 52W-Hoch, GD200-Crossover oder feste Lookbacks.
        # Wenn kein Einstieg gespeichert: kein Signal (manuell setze_einstieg() aufrufen).
        ...
        if (abstand_hoch is not None and
            ...
            # Teilverkauf als ausgelöst markieren
            try:
                conn_tv = sqlite3.connect("journal.db")
                conn_tv.execute(
                    "UPDATE position_tracking SET teilverkauf_ausgeloest = 1 WHERE ticker = ?",
                    (ticker,)
                )
                conn_tv.commit()
                conn_tv.close()
...
def _ensure_ranking_columns(conn):
    ...
    Löscht NICHT die Trades-Tabelle — nur ranking_history wird ggf. neu gebaut.
    """
    try:
        cur = conn.cursor()
        cur.execute("PRAGMA table_info(ranking_history)")
        cols = {row[1] for row in cur.fetchall()}

        # Falls die Tabelle alte Struktur hat (kein kw/jahr/rank_pos), neu aufbauen
        muss_neu = not cols or not {'kw', 'jahr', 'rank_pos'}.issubset(cols)
        if muss_neu:
            conn.execute("DROP TABLE IF EXISTS ranking_history")
            conn.execute('''CREATE TABLE ranking_history (
        ...
        # Im schlimmsten Fall: Tabelle neu anlegen
        try:
            conn.execute("DROP TABLE IF EXISTS ranking_history")
            conn.execute('''CREATE TABLE ranking_history (
                id        INTEGER PRIMARY KEY AUTOINCREMENT,
...
def init_stabilitaet_db():
    """DB-Init mit Migrations-Schutz. Stürzt nie ab."""
    try:
        conn = sqlite3.connect("journal.db")
        # ranking_history mit Migrations-Check
        _ensure_ranking_columns(conn)
        # position_tracking mit teilverkauf_ausgeloest
        conn.execute('''CREATE TABLE IF NOT EXISTS position_tracking (
            id                    INTEGER PRIMARY KEY AUTOINCREMENT,
            ticker                TEXT    UNIQUE,
            einstieg_datum        TEXT,
            einstieg_kurs         REAL,
            hoechstkurs           REAL,
            teilverkauf_ausgeloest INTEGER DEFAULT 0,
            letzte_aktualisierung TEXT
        )''')
        # Migration: teilverkauf_ausgeloest nachträglich anlegen falls Tabelle alt
        try:
            conn.execute("ALTER TABLE position_tracking ADD COLUMN teilverkauf_ausgeloest INTEGER DEFAULT 0")
            conn.commit()
        except Exception:
            pass  # Spalte existiert bereits — kein Fehler
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def speichere_ranking(scan_df):
    """
    Aktuelles Ranking speichern — pro ISO-Woche genau einmal.
    GSheets primär → SQLite als Fallback.
    """
    heute = datetime.now().strftime('%Y-%m-%d')
    ...
    if _gsheets_ok():
        try:
            # Alte Einträge dieser Woche löschen
    ...
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        conn.execute("DELETE FROM ranking_history WHERE kw=? AND jahr=?", (kw, jahr))
        for i, row in scan_df.iterrows():
            conn.execute(
                "INSERT INTO ranking_history (datum, kw, jahr, ticker, rank_pos, score) VALUES (?,?,?,?,?,?)",
                (heute, kw, jahr, row['Ticker'], i + 1, float(row.get('Score ⭐', 0) or 0))
            )
        conn.commit()
        conn.close()
    except Exception:
...
def pruefe_stabilitaet(ticker, top_n):
    """
    Echte 2-Wochen-Bestätigung via ISO-Kalenderwochen.
    GSheets primär → SQLite als Fallback.
    ...
    try:
        ...
        cur.execute(
            "SELECT kw, jahr FROM ranking_history "
            "WHERE datum >= ? GROUP BY jahr, kw ORDER BY jahr ASC, kw ASC",
            (vier_wochen_ago,)
        )
        alle_wochen_rows = cur.fetchall()

        if len(alle_wochen_rows) < 2:
            conn.close()
            return 'neu', 0

        vorletzte_woche = alle_wochen_rows[-2]
        letzte_woche    = alle_wochen_rows[-1]

        def hole_rank_in_woche(kw, jahr):
            cur.execute(
                "SELECT rank_pos FROM ranking_history "
                "WHERE ticker = ? AND kw = ? AND jahr = ? LIMIT 1",
                (ticker, kw, jahr)
            )
            row = cur.fetchone()
            return row[0] if row else None

        rank_letzte    = hole_rank_in_woche(letzte_woche[0],    letzte_woche[1])
        rank_vorletzte = hole_rank_in_woche(vorletzte_woche[0], vorletzte_woche[1])
        conn.close()
...
TRADES_CSV = "trades_backup.csv"
TRADES_COLS = ['id','datum','ticker','name','typ','kurs','betrag',
               'begruendung','ausstieg_trigger','ausstieg_kurs',
               'ausstieg_datum','pnl','notizen']

def trade_add(ticker, name, typ, kurs, betrag, grund, trigger):
    """
    Trade speichern — GSheets primär, SQLite + CSV als Fallback.
    """
    heute = datetime.now().strftime('%Y-%m-%d')
    ...
    try:
        ...
        cur.execute(
            ...
            (heute, ticker.upper(), name, typ, kurs, betrag, grund, trigger)
        )
        conn.commit()
        # CSV-Backup
        try:
            import csv, os
            schreibe_header = not os.path.exists(TRADES_CSV)
            with open(TRADES_CSV, 'a', newline='', encoding='utf-8') as f:
                w = csv.writer(f)
                if schreibe_header:
                    w.writerow(TRADES_COLS)
                w.writerow([cur.lastrowid, heute, ticker.upper(), name, typ,
                            kurs, betrag, grund, trigger, None, None, None, None])
        except Exception:
            pass
        conn.close()
    except Exception:
...
def trades_load():
    """
    Trades laden — GSheets primär, SQLite → CSV als Fallback.
    """
    # GSheets
    if _gsheets_ok():
        df = gs_load_df("trades_journal")
        if not df.empty:
            # Spalten auf einheitliche Namen normalisieren
...
def checklist_save(vix, osc, pos, notizen):
    """Checkliste speichern — GSheets primär, SQLite Fallback."""
    heute = datetime.now().strftime('%Y-%m-%d')
...
st.markdown(
    ...
    f'background:{"#f0fdf4" if _gsheets_ok() else "#f8fafc"};'
    f'color:{"#16a34a" if _gsheets_ok() else "#9ca3af"};'
    f'border:1px solid {"#86efac" if _gsheets_ok() else "#e2e8f0"}">'
    f'{"☁️ GSheets" if _gsheets_ok() else "💾 Lokal"}</span>'
