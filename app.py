...
def breadth_faktor(osc):
    ...
    Beispiel: 60% → 0.60, 45% → 0.45, 30% → 0.30
    Keine festen Stufen mehr — direkte Proportionalität.
    Anzeigetext und Farbe für UI werden trotzdem abgeleitet.
    """
    if osc is None:
        return 1.0, "Keine Daten", "#718096", True
    faktor = osc / 100.0
    neue_kauf_erlaubt = osc >= 40  # Käufe nur ab 40% Breadth
    if osc >= 60:
        text, farbe = "Volle Stärke", "#16a34a"
    elif osc >= 40:
        text, farbe = "Reduziert", "#ca8a04"
    elif osc >= 30:
        text, farbe = "Stark reduziert — keine Käufe", "#ea580c"
    else:
        text, farbe = "Positionen abbauen", "#dc2626"
        neue_kauf_erlaubt = False
    return faktor, text, farbe, neue_kauf_erlaubt

def vix_korrektur(vix, basis_pos):
    """
    VIX-Dämpfung (nach Breadth):
    VIX < 25  → kein Einfluss
    VIX 25–40 → proportionale Dämpfung: Faktor = 1 − (VIX−25)/30
    VIX > 40  → keine neuen Käufe
    """
    if vix is None or vix < 25:
        return basis_pos, True
    if vix <= 40:
        daempf = 1.0 - (vix - 25) / 30.0   # linear: 25→1.0, 40→0.5
        gedaempft = max(0, round(basis_pos * daempf))
        return gedaempft, True
    return 0, False

def berechne_positionen(top_n, osc, vix):
    """
    Positionsanzahl = Top-N × Breadth(osc/100), danach VIX-Dämpfung.
    """
...
def scan(ticker, min_vol=5_000_000, min_kurs=10.0):
    """
    Momentum-System v3.1

    FILTER:
    - Kurs > min_kurs (Standard: 10)
    - Ø Volumen (60T) ≥ min_vol
    - Kurs > GD200 (Pflichtfilter)

    RANKING (z-Score normalisiert, dann gewichtet):
    - MOM260  = 12M-Momentum AQR-Lag  → 40%
    - MOMJT   = 6M-Jahrestrend AQR-Lag → 30%
    - GD130   = Abstand zur GD130      → 30%
    Normalisierung erfolgt erst über alle Kandidaten hinweg
    (diese Funktion gibt die Rohwerte zurück; Normalisierung in berechne_scores_normalisiert)

    EXIT-SIGNALE:
    - Teilverkauf: Kurs −20% vom Einstiegs-Trendhoch (nicht 52W-Hoch)
    - Vollverkauf: Kurs 20–30% unter Ausbruchsniveau (GD200 × 1.05 als Proxy)
                   oder 2W unter Strukturlevel (via Stabilitätsprüfung)
    """
    try:
        ...
        # ── AUSBRUCHSNIVEAU als Strukturlevel-Proxy ──────
        # Ausbruch = Kurs beim Überkreuzen des GD200 (letzter Aufwärts-Crossover)
        # Approximation: GD200 × 1.05 (5% Puffer über GD200 = typischer Ausbruchsbereich)
        ausbruch_niveau = gd200 * 1.05

        # ── TRENDHOCH seit Aufwärtstrend-Start ───────────
        # Trendhoch = Maximum seit dem Kurs zuletzt den GD200 überkreuzt hat
        # Approximation: Höchstkurs der letzten 65 Handelstage (≈ 3 Monate Trendhoch)
        trendhoch = h['Close'].tail(65).max()
        abstand_trendhoch = round(((k - trendhoch) / trendhoch) * 100, 1)

        # ── EXIT-SIGNALE ──────────────────────────────────
        exit_signal = ""
        exit_typ = ""

        # Teilverkauf: −20% vom Trendhoch (seit Einstieg / Trendhoch, NICHT 52W-Hoch)
        if abstand_trendhoch <= -20:
            exit_signal = "⚠️ TEILVERKAUF 50%"
            exit_typ = "teilverkauf"

        # Vollverkauf Typ 1: Kurs 20–30% unter Ausbruchsniveau
        abstand_ausbruch = round(((k - ausbruch_niveau) / ausbruch_niveau) * 100, 1)
        if abstand_ausbruch <= -20:
            exit_signal = "🔴 VOLLVERKAUF (unter Ausbruch)"
            exit_typ = "vollverkauf"

        # Vollverkauf Typ 2: Kurs unter GD200 (bereits durch Filter ausgeschlossen,
        # aber falls knapp über GD200: Warnsignal)
        if gd200_abstand < 3:
            if exit_typ != "vollverkauf":
        ...
        return {
            ...
            'MOM260_roh':       mom260,      # Rohwert für Normalisierung
            'MOMJT_roh':        momjt,       # Rohwert für Normalisierung
            'GD130_abst_roh':   gd130_abstand,  # Rohwert für Normalisierung
            'MOM260 %':         mom260,
            'MOMJT %':          momjt,
            'Trendhoch':        round(trendhoch, 2),
            'Abst. Trendhoch%': abstand_trendhoch,
...
def init_stabilitaet_db():
    ...
    # Einstiegskurse für Teilverkauf/Vollverkauf-Tracking
...
def speichere_ranking(scan_df):
    ...
    # Alte Einträge dieser Woche löschen (Überschreiben bei mehrfachem Speichern)
    conn.execute("DELETE FROM ranking_history WHERE kw=? AND jahr=?", (kw, jahr))
    for i, row in scan_df.iterrows():
        conn.execute(
            "INSERT INTO ranking_history (datum, kw, jahr, ticker, rank, score) VALUES (?,?,?,?,?,?)",
            (heute, kw, jahr, row['Ticker'], i+1, row.get('Score ⭐', 0))
        )
    conn.commit()
    conn.close()
...
def pruefe_stabilitaet(ticker, top_n):
    ...
    KAUF:   Ticker war in den letzten 2 *verschiedenen* ISO-Wochen in Top-N.
    VERKAUF: Ticker war in den letzten 2 *verschiedenen* ISO-Wochen NICHT in Top-N.

    Gibt zurück: (status, anzahl_wochen_im_system)
      status: 'bestätigt' | 'beobachten' | 'exit_2w' | 'neu'
    """
    init_stabilitaet_db()
    conn = sqlite3.connect("journal.db")

    # Letzte 4 Wochen an Daten laden (für 2-Wochen-Logik ausreichend)
    heute = datetime.now()
    vier_wochen_ago = (heute - timedelta(weeks=4)).strftime('%Y-%m-%d')

    # Alle Wochen in denen dieser Ticker im Ranking war
    alle_wochen = pd.read_sql(
        "SELECT DISTINCT kw, jahr, rank FROM ranking_history "
        "WHERE ticker=? AND datum>=? ORDER BY jahr, kw",
        conn, params=(ticker, vier_wochen_ago)
    )
    # Alle Wochen die überhaupt gespeichert wurden (unabhängig vom Ticker)
    alle_ranking_wochen = pd.read_sql(
        "SELECT DISTINCT kw, jahr FROM ranking_history "
        "WHERE datum>=? ORDER BY jahr, kw",
        conn, params=(vier_wochen_ago,)
    )
    conn.close()

    if alle_ranking_wochen.empty:
        return 'neu', 0

    total_wochen = len(alle_ranking_wochen)

    if alle_wochen.empty:
        # Ticker kam in keiner der gespeicherten Wochen vor
        if total_wochen >= 2:
            return 'exit_2w', 0
        return 'neu', 0

    # Wochen in Top-N
    in_top_n = alle_wochen[alle_wochen['rank'] <= top_n]
    anzahl_in_top_n = len(in_top_n)

    # Letzte 2 gespeicherte Wochen
    letzte_2_wochen = alle_ranking_wochen.tail(2)
    letzte_2_set = set(zip(letzte_2_wochen['jahr'], letzte_2_wochen['kw']))

    # In wie vielen der letzten 2 Wochen war der Ticker in Top-N?
    in_top_n_set = set(zip(in_top_n['jahr'], in_top_n['kw']))
    in_letzten_2 = len(letzte_2_set & in_top_n_set)

    if in_letzten_2 >= 2:
        return 'bestätigt', anzahl_in_top_n
    elif in_letzten_2 == 1:
        return 'beobachten', anzahl_in_top_n
    elif total_wochen >= 2:
        # In den letzten 2 Wochen gar nicht in Top-N → Verkaufssignal
        return 'exit_2w', anzahl_in_top_n
    else:
        return 'neu', anzahl_in_top_n
...
with tab3:
    ...
    if 'universe_df' not in st.session_state:
        ...
        if 'Ticker' in uni_df.columns:
            ...
            if scan_btn:
                results = []
                gesehene_ticker = set()   # Deduplizierung
                prog2 = st.progress(0)
                status2 = st.empty()
                ...
                for i, t in enumerate(uni_tickers):
                    ...
                    # ── Duplikat-Filter ──────────────────────────
                    if t in gesehene_ticker:
                        continue
                    gesehene_ticker.add(t)
                ...
                if results:
                    ...
                    cols_show = ['#', 'Name', 'Ticker', 'Kurs', 'GD200', 'GD200 Abst.%',
                                 ...
                                 'Score ⭐', 'Trendhoch', 'Abst. Trendhoch%',
                    ...
                    with st.expander("📋 Kauf- & Verkaufs-Regeln"):
                        st.markdown(
                            ...
                            "- ✅ Kurs über GD200\n"
                            "- ✅ Im Top-Ranking (Score ⭐, z-Score normalisiert)\n"
                            "- ✅ 2 ISO-Wochen bestätigt (Spalte: Stabilität)\n"
                            "- ✅ Positionslimit nicht erreicht\n"
                            "- ✅ Sektorregel: max. 2 pro Sektor\n"
                            "- ✅ Neue Käufe erlaubt (Breadth ≥ 40% und VIX ≤ 40)\n\n"
                            "**Exit-Regeln:**\n"
                            "- ⚠️ **Teilverkauf 50%:** Kurs −20% vom Trendhoch (letztes 65T-Hoch) → halbe Position verkaufen\n"
                            "- 🔴 **Vollverkauf Typ 1:** Kurs 20–30% unter Ausbruchsniveau (GD200 × 1.05)\n"
                            "- 🔴 **Vollverkauf Typ 2:** 2 ISO-Wochen außerhalb Top-N (Spalte: Stabilität = 🔴 EXIT)\n"
                            "- ⚠️ **GD200-Nähe:** Kurs < 3% über GD200 → Vollverkauf prüfen\n\n"
                            "**Kein Sofort-Exit** — immer 2-Wochen-Bestätigung abwarten.\n"
