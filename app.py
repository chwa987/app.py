...
def pruefe_stabilitaet(ticker, top_n):
    ...
    Verwendet cursor.fetchall() statt pd.read_sql — robust gegen alle pandas/SQLite-Versionen.

    KAUF:    Ticker war in 2 verschiedenen ISO-Wochen in Top-N → 'bestätigt'
    VERKAUF: Ticker war in 2 verschiedenen ISO-Wochen NICHT in Top-N → 'exit_2w'
    """
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        ...
        # ── Alle gespeicherten Wochen im Zeitraum ─────────
        cur.execute(
            "SELECT kw, jahr FROM ranking_history WHERE datum >= ? GROUP BY jahr, kw ORDER BY jahr, kw",
            (vier_wochen_ago,)
        )
        alle_wochen_rows = cur.fetchall()   # [(kw, jahr), ...]

        if not alle_wochen_rows:
            conn.close()
            return 'neu', 0

        total_wochen = len(alle_wochen_rows)

        # ── Wochen in denen dieser Ticker im Ranking war ──
        cur.execute(
            "SELECT kw, jahr, rank_pos FROM ranking_history "
            "WHERE ticker = ? AND datum >= ? ORDER BY jahr, kw",
            (ticker, vier_wochen_ago)
        )
        ticker_rows = cur.fetchall()   # [(kw, jahr, rank_pos), ...]
        conn.close()

        # Wochen in denen Ticker in Top-N war
        ticker_in_top_n = {(kw, jahr) for kw, jahr, rank_pos in ticker_rows if rank_pos <= top_n}
        anzahl_in_top_n = len(ticker_in_top_n)

        # Letzte 2 gespeicherte Wochen als Set
        letzte_2 = set(alle_wochen_rows[-2:]) if total_wochen >= 2 else set(alle_wochen_rows)

        in_letzten_2 = len(letzte_2 & ticker_in_top_n)

        if in_letzten_2 >= 2:
            return 'bestätigt', anzahl_in_top_n
        elif in_letzten_2 == 1:
            return 'beobachten', anzahl_in_top_n
        elif total_wochen >= 2 and anzahl_in_top_n == 0:
            return 'exit_2w', 0
        else:
            return 'neu', anzahl_in_top_n

    except Exception:
        # Defensiv: niemals die App crashen lassen
        return 'neu', 0
