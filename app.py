...
def _get_hoechstkurs(ticker):
    """Holt den gespeicherten Höchstkurs seit Einstieg aus position_tracking."""
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        cur = conn.cursor()
        cur.execute(
            "SELECT hoechstkurs FROM position_tracking WHERE ticker = ? LIMIT 1",
            (ticker,)
        )
        row = cur.fetchone()
        conn.close()
...
def _update_hoechstkurs(ticker, kurs_aktuell):
    """
    Aktualisiert den Höchstkurs in position_tracking falls aktueller Kurs neues Hoch ist.
    Legt den Eintrag an falls noch nicht vorhanden.
    """
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        ...
        if row is None:
            conn.execute(
                ...
                "(ticker, einstieg_datum, einstieg_kurs, hoechstkurs, letzte_aktualisierung) "
                "VALUES (?, ?, ?, ?, ?)",
            ...
            if kurs_aktuell > bisheriges_hoch:
                conn.execute(
                    "UPDATE position_tracking SET hoechstkurs = ?, letzte_aktualisierung = ? "
                    "WHERE ticker = ?",
...
def scan(ticker, min_vol=5_000_000, min_kurs=10.0):
    ...
    try:
        ...
        # ── AUSBRUCHSNIVEAU (Strukturlevel-Proxy) ────────
        # GD200 × 1.05 = typischer Ausbruchsbereich über dem gleitenden Durchschnitt
        ausbruch_niveau = gd200 * 1.05

        # ── TRENDHOCH (Fallback für Teilverkauf) ─────────
        # Lokales Hoch seit letztem GD200-Crossover:
        # Suche den letzten Tag, an dem der Kurs den GD200 von unten überkreuzt hat,
        # und nehme das Maximum seitdem als Trendhoch.
        close_arr = h['Close'].values
        gd200_arr = h['Close'].rolling(200).mean().values
        crossover_idx = None
        for i in range(len(close_arr) - 2, 0, -1):
            if (close_arr[i] > gd200_arr[i] and
                    close_arr[i-1] <= gd200_arr[i-1] and
                    not np.isnan(gd200_arr[i-1])):
                crossover_idx = i
                break
        if crossover_idx is not None:
            trendhoch_fallback = float(h['Close'].iloc[crossover_idx:].max())
        else:
            # Kein Crossover gefunden: letztes lokales Hoch (65 Handelstage ≈ 3 Monate)
            trendhoch_fallback = float(h['Close'].tail(65).max())

        # ── HOCHstkurs seit Einstieg aus position_tracking ──
        # Wenn Position bekannt: nutze gespeichertes Hoch seit Einstieg
        # Sonst: Trendhoch als Fallback
        hoch_seit_einstieg = _get_hoechstkurs(ticker)
        if hoch_seit_einstieg is None or hoch_seit_einstieg < k:
            # Noch nicht getrackt oder Kurs über bisherigem Hoch → Trendhoch als Proxy
            hoch_fuer_teilverkauf = trendhoch_fallback
        else:
            hoch_fuer_teilverkauf = hoch_seit_einstieg

        # Hoch in DB aktualisieren falls nötig
        _update_hoechstkurs(ticker, k)

        abstand_hoch = round(((k - hoch_fuer_teilverkauf) / hoch_fuer_teilverkauf) * 100, 1)

        # ── EXIT-SIGNALE ──────────────────────────────────
        exit_signal = ""
        exit_typ = ""

        # Teilverkauf: −20% vom Hoch seit Einstieg (oder Trendhoch als Fallback)
        if abstand_hoch <= -20:
            exit_signal = "⚠️ TEILVERKAUF 50% (−20% vom Hoch seit Einstieg)"
            exit_typ = "teilverkauf"

        # Vollverkauf Typ 1: ≥ 20% unter Ausbruchsniveau (Wochenbasis bestätigt)
        abstand_ausbruch = round(((k - ausbruch_niveau) / ausbruch_niveau) * 100, 1)
        if abstand_ausbruch <= -20:
            exit_signal = "🔴 VOLLVERKAUF (≥20% unter Ausbruch — 2W bestätigen)"
            exit_typ = "vollverkauf"

        # GD200-Nähe: Warnsignal (unter 3% über GD200, kein Vollverkauf-Override)
        if gd200_abstand < 3 and exit_typ not in ("vollverkauf", "teilverkauf"):
        ...
        return {
            ...
            'MOM260 %':         mom260,
            'MOMJT %':          momjt,
            'Trendhoch':        round(hoch_fuer_teilverkauf, 2),
            'Abst. Hoch%':      abstand_hoch,
            'Ausbruch Niveau':  round(ausbruch_niveau, 2),
...
def speichere_ranking(scan_df):
    """Aktuelles Ranking in DB speichern — pro ISO-Woche genau einmal."""
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        heute = datetime.now().strftime('%Y-%m-%d')
        ...
        conn.execute(
            "DELETE FROM ranking_history WHERE kw=? AND jahr=?",
            (kw, jahr)
        )
        for i, row in scan_df.iterrows():
            conn.execute(
                "INSERT INTO ranking_history "
                "(datum, kw, jahr, ticker, rank_pos, score) VALUES (?,?,?,?,?,?)",
                (heute, kw, jahr, row['Ticker'], i + 1, float(row.get('Score ⭐', 0) or 0))
            )
        conn.commit()
        conn.close()
    except Exception:
        pass  # Niemals crashen

def pruefe_stabilitaet(ticker, top_n):
    """
    Echte 2-Wochen-Bestätigung via ISO-Kalenderwochen.
    Verwendet cursor.fetchall() — robust gegen alle pandas/SQLite-Versionen.

    KAUF:    Ticker war in den letzten 2 vollständigen ISO-Wochen BEIDE in Top-N → 'bestätigt'
    VERKAUF: Ticker war in den letzten 2 vollständigen ISO-Wochen BEIDE NICHT in Top-N → 'exit_2w'

    Wichtig: "letzte" und "vorletzte" Woche werden explizit einzeln geprüft,
    keine Summenlogik, kein "irgendwann in 4 Wochen drin gewesen".
    """
    try:
        init_stabilitaet_db()
        conn = sqlite3.connect("journal.db")
        ...
        # ── Alle gespeicherten Wochen (chronologisch) ─────
        cur.execute(
            "SELECT kw, jahr FROM ranking_history "
            "WHERE datum >= ? GROUP BY jahr, kw ORDER BY jahr ASC, kw ASC",
            (vier_wochen_ago,)
        )
        alle_wochen_rows = cur.fetchall()   # [(kw, jahr), ...]

        if len(alle_wochen_rows) < 2:
            conn.close()
            return 'neu', 0

        # ── Letzte 2 vollständige Wochen explizit benennen ─
        vorletzte_woche = alle_wochen_rows[-2]   # (kw, jahr)
        letzte_woche    = alle_wochen_rows[-1]   # (kw, jahr)

        # ── Für diesen Ticker: rank_pos in jeder der letzten 2 Wochen ──
        def hole_rank_in_woche(kw, jahr):
            ...
            row = cur.fetchone()
            return row[0] if row else None  # None = nicht im Ranking dieser Woche
        ...
        # In Top-N? (None = nicht drin = außerhalb)
        ...
        # Anzahl Wochen gesamt in Top-N (für Anzeige)
        anzahl = sum([1 for r in [rank_letzte, rank_vorletzte]
                      if r is not None and r <= top_n])

        # ── Kauf-Bestätigung: BEIDE letzten 2 Wochen in Top-N ──
        if in_top_n_letzte and in_top_n_vorletzte:
            return 'bestätigt', anzahl

        # ── Verkauf-Signal: BEIDE letzten 2 Wochen NICHT in Top-N ──
        if not in_top_n_letzte and not in_top_n_vorletzte:
            return 'exit_2w', 0

        # ── Beobachten: eine Woche drin, eine nicht ──
                ...
                if results:
                    ...
                    with st.expander("📋 Kauf- & Verkaufs-Regeln"):
                        st.markdown(
                            ...
                            "- ✅ Kurs über GD200\n"
                            "- ✅ Im Top-Ranking (Score ⭐, z-Score normalisiert: MOM260×40% + MOMJT×30% + GD130×30%)\n"
                            "- ✅ 2 vollständige ISO-Wochen in Top-N bestätigt (Spalte: Stabilität)\n"
                            "- ✅ Positionslimit nicht erreicht (Top-N × Breadth, dann VIX-Dämpfung)\n"
                            ...
                            "(aus position_tracking; Fallback: lokales Trendhoch seit GD200-Crossover)\n"
                            "- 🔴 **Vollverkauf Typ 1:** Kurs ≥ 20% unter Ausbruchsniveau (GD200 × 1.05) "
                            "— 2 Wochen bestätigen bevor Verkauf\n"
                            "- 🔴 **Vollverkauf Typ 2:** 2 vollständige ISO-Wochen außerhalb Top-N "
                            "(Stabilität-Spalte zeigt '🔴 EXIT: 2W außerhalb Top-N')\n"
                            "- ⚠️ **GD200-Nähe:** Kurs < 3% über GD200 → Vollverkauf manuell prüfen\n\n"
